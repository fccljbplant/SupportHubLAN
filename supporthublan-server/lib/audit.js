...
function add(db, entry) {
  try {
    ...
    const auditEntry = {
      ...
      command: (entry.command || '').substring(0, 2000),
      ...
      output_summary: (entry.outputSummary || entry.output || '').substring(0, 2000),
      full_output: (entry.fullOutput || entry.output || '').substring(0, 5000),
